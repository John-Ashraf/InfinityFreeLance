...
namespace Api.Service.Implementation
{
    public class FileService : IFileService
    {
        ...
        public async Task<string> DeleteImage(string Location, string fileName)
        {
            var path = Path.Combine(_webHostEnvironment.WebRootPath, Location, fileName);
            ...
            if (System.IO.File.Exists(path))
            {
                try
                {
                    ...
                    return "ImageDeletedSuccessfully";
                }
                catch (Exception ex)
                {
                    // Log the exception if needed
                    return $"FailedToDeleteImage: {ex.Message}";
                }
            }
            else
            {
